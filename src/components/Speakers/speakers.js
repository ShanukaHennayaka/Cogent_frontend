...
const Speakers = () => {
    ...
    useEffect(() => {
        gsap.set(".speakerCard", { opacity: 1, y: 0 });

        gsap.utils.toArray(".speakerCard").forEach((card, i) => {
            gsap.from(card, {
                scrollTrigger: {
                    trigger: card,
                    start: "top 80%",
    ...
    }, [selectedSpeaker, showAll]);

    return (
        <section id="speakers" className="section" ref={sectionRef}>
            <div className="section-header">
                <h2 className="gradientTitle">Meet Our Speakers</h2>
                <p className="sectionDesc">
                    Industry leaders shaping the future of banking
                </p>
            </div>
                ...
                {visibleSpeakers.map((speaker, index) => (
                    ...
                    >
                        ...
                        <div className="speakerInfo">
                            <h3>{speaker.name}</h3>
                            <p>{speaker.title}</p>
                            <button className="viewBioBtn">View Bio</button>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {speakers.length > 6 && (
                ...
                >
                    {showAll ? 'Show Less' : 'Load More Speakers'}
                    <span className={`arrow ${showAll ? 'up' : 'down'}`}></span>
                </button>
            )}

            {selectedSpeaker && (
                <div className="speakerModal" onClick={() => setSelectedSpeaker(null)}>
                    <div className="modalContent" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modalHeader">
                            ...
                            <div>
                                ...
                                <p className="modalTitle">{selectedSpeaker.title}</p>
                            </div>
                        </div>
                        <div className="modalBody">
                            {selectedSpeaker.description ? (
                                ...
                                <p className="noBio">Speaker bio coming soon</p>
                            )}
                        </div>
                    </div>
                </div>
